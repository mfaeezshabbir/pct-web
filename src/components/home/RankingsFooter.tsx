...
export default function RankingsFooter() {
  ...
  const TeamRankCard = ({
    ...
    return (
      <div className="col-span-1 md:col-span-3 h-auto md:h-64 relative bg-pct-green dark:bg-gradient-to-br dark:from-pct-green/20 dark:to-black/50 rounded-2xl p-6 md:px-12 flex flex-col md:flex-row items-center justify-between overflow-visible shadow-2xl group border border-white/10 gap-6 md:gap-0 mt-20 md:mt-0">
        {/* Left Content: Title & Name */}
        <div className="relative z-30 flex flex-col justify-center items-center md:items-start h-auto md:h-full w-full md:w-1/3 text-center md:text-left order-2 md:order-1">
          <span className="text-xs font-bold text-pct-gold uppercase tracking-[0.2em] mb-2">
            {title}
          </span>
          <h3 className="text-5xl md:text-7xl font-oswald font-bold text-white italic tracking-tight mb-4 drop-shadow-lg">
            PAKISTAN
          </h3>
        ...
        {image && (
          <div className="static md:absolute inset-x-0 bottom-0 top-[-20%] md:top-[-40%] z-20 pointer-events-none flex justify-center items-end overflow-visible order-1 md:order-2 -mt-24 md:mt-0 mb-4 md:mb-0">
            <div className="relative h-[250px] md:h-full w-full max-w-lg">
              <img
        ...
        <div className="relative z-30 flex flex-col items-center md:items-end justify-center h-auto md:h-full w-full md:w-1/3 order-3">
          {/* Rank Box */}
          <div className="relative bg-black/20 backdrop-blur-sm rounded-2xl w-28 h-28 md:w-32 md:h-32 flex flex-col items-center justify-center border border-white/10 shadow-xl">
